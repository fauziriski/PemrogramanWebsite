...
$result1 = mysqli_query($conn,"SELECT * FROM bis1 WHERE 
		asal = '$asal' AND
		tujuan = '$tujuan' AND
		keberangkatan = '$keberangkatan'AND
		kursi LIKE '%A'");

$result2 = mysqli_query($conn,"SELECT * FROM bis1 WHERE 
		asal = '$asal' AND
		tujuan = '$tujuan' AND
		keberangkatan = '$keberangkatan' AND
		kursi LIKE '%B'");

$result3 = mysqli_query($conn,"SELECT * FROM bis1 WHERE 
		asal = '$asal' AND
		tujuan = '$tujuan' AND
		keberangkatan = '$keberangkatan' AND
		kursi LIKE '%C'");

$result4 = mysqli_query($conn,"SELECT * FROM bis1 WHERE 
		asal = '$asal' AND
		tujuan = '$tujuan' AND
		keberangkatan = '$keberangkatan' AND
		kursi LIKE '%D'");
...
<body>
	...
	<table>
		...
		<?php foreach ($result1 as $dat) :?>
			...
			<?php else :?>
				<td class="ada"><a href="isi.php?id=<?= $dat["id"]; ?>"><?=$dat["kursi"] ?></a></td>
			<?php endif; ?>
